...
appScreens2 = [
    Screen2 {
       scrName    = "accounts"
      ,scrHandler = handleAccountsScreen appScreens2
      ,scrDraw    = drawAccountsScreen
      ,scrLoad    = loadAccountsScreen
      ,scrItems   = L.list "" drawAccountsItem []
      }
   ,Screen2 {
       scrName    = "register"
      ,scrHandler = handleRegisterScreen
      ,scrDraw    = drawRegisterScreen
      ,scrLoad    = loadRegisterScreen
      ,scrItems   = L.list "" drawRegisterItem []
      }
  ]
...
runBrickUi opts j = do
  ...
  let
    ...
    st :: AppState
    st = screenEnter d args (head appScreens2)
         AppState{
            aopts=opts
           ,aargs=args
           ,ajournal=j
    ...
    app = M.App {
      ...
      , M.appHandleEvent  = \st ev -> (scrHandler $ locScreen $ aloc st) st ev
      , M.appDraw         = \st -> (scrDraw $ locScreen $ aloc st) st
      }
